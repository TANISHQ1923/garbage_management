...
@Composable
fun DriverCollectionHistoryScreen(
            ...
            TopAppBar(
                title = {
                    Column {
                        ...
                        Text(
                            text = "${uiState.historyComplaints.size} completed cleanups",
        ...
        Column(
            ...
            LazyRow(
                ...
                items(HistoryFilter.entries) { filter ->
                    val isSelected = uiState.selectedHistoryFilter == filter
                    FilterChip(
                        selected = isSelected,
                        onClick = { driverViewModel.setHistoryFilter(filter) },
                        label = {
                            Text(
            ...
            if (uiState.isLoading) {
            ...
            } else if (uiState.historyComplaints.isEmpty()) {
                // EMPTY STATE
                Box(
                    modifier = Modifier
                        .fillMaxSize()
                    ...
                    Column(
                        ...
                        Text(
                            text = "No completed collections yet",
                        ...
                        Text(
                            text = "Collections marked as Cleaned or Verified will appear here.",
                ...
                LazyColumn(
                    ...
                    items(uiState.historyComplaints, key = { it.id }) { complaint ->
                        HistoryTaskCard(
                            complaint = complaint,
...
fun HistoryTaskCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            // FOOTER: Completed timestamp + Photo indicators
            Row(
                modifier = Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.SpaceBetween,
                verticalAlignment = Alignment.CenterVertically
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Spacer(modifier = Modifier.width(4.dp))
                    Text(
                        text = "Completed: ${formatTimestamp(complaint.updatedAt)}",
                        style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.outline
                    )
                }

                // Photo evidence indicator tags
                Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                    if (!complaint.beforeCleaningImageUri.isNullOrBlank()) {
                        EvidenceTag(label = "Before", icon = Icons.Default.CameraAlt)
                    }
                    if (!complaint.afterCleaningImageUri.isNullOrBlank()) {
                        EvidenceTag(label = "After", icon = Icons.Default.Image)
                    }
                }
            }
        }
